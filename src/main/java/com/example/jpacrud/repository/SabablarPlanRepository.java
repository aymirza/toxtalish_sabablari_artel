...
public interface SabablarPlanRepository extends JpaRepository<SabablarPlan,Long> {
    ...
    @Query("SELECT sp FROM SabablarPlan sp WHERE CONCAT(sp.uchastka,sp.nachalnik,sp.master,sp.brigadir,sp.smena,sp.toxtalishsababi,sp.toxtalishnuktasi,sp.toxtalishdate,sp.starttime,sp.finishtime,sp.toxtaganvaqt) LIKE %?1%")
